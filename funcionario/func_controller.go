...
func UpdateFunc_C(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&funcEncontrado); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Dados inválidos"})
		return
	}

	// Validação do nome
	if !ValidarNome(funcEncontrado.Nome) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Nome inválido. Use apenas letras e espaços"})
		return
	}

	// Validação do cargo
	if !ValidarCargo(funcEncontrado.Cargo) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Cargo inválido. Use apenas letras e espaços"})
		return
	}

	// Validação do CPF
	if !ValidarCPF(funcEncontrado.Cpf) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "CPF inválido. Digite apenas números ou no formato 000.000.000-00"})
		return
	}
	funcEncontrado.Cpf = PadronizarCPF(funcEncontrado.Cpf) // salva sem pontuação

	// Validação da idade
	if !ValidarIdade(funcEncontrado.Idade) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Idade inválida. Deve ser um número maior que zero"})
		return
	}

	// Atualiza no banco
	if err := UpdateFunc(funcEncontrado); err != nil {
